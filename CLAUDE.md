...
- **Cloud Functions exist and own ELO + league (LP).** `functions/src/index.ts` ships `onMatchFinished` (ELO + LP **authoritative**, ranked-only — casual/friend matches are ignored), `resetDailyGames`, `onFriendRequest` (push), and `validateAnswer`. The client reports `scores` + `winnerId` and sets `status: finished`; the function resolves ELO/league for **both** players once. Deploy with `firebase deploy --only functions`.
- **`DESIGN.md` (root) is from the geography era** and now carries a deprecation notice; the real theme is **"Stadium Arena"** (forces dark mode) in `core/theme/app_colors.dart` and `Pantallas/DESIGN.md`. `README.md` and `PROGRESS.md` have been refreshed to current state; **`PROGRESS.md` remains the source of truth**.
- **`QuestionType` is football-only** (`domain/entities/question.dart`): the geography types (`area`, `population`, `river`, …) were removed during fork cleanup. ~30% of questions are converted to type-answer mode by `_convertToTypeAnswer`.
- **Tests exist (~100).** `test/core`, `test/data`, `test/widgets`, `test/services` cover utils, `_enrichOptions`, and key widgets; the old counter-template `widget_test.dart` was deleted. Caveat: `home_screen_test.dart` currently times out on `pumpAndSettle` (background video/animation). Still no `integration_test/` dir despite the pubspec dependency.
