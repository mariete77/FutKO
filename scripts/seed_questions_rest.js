// FutKO — Seed Script para Firestore (REST API)
// Ejecutar con: node scripts/seed_questions_rest.js
// No requiere npm install - usa fetch nativo de Node.js 18+

const projectId = 'futko-battle';
const apiKey = 'AIzaSyAprAmk1hPhlNbkQhgTSjIeEfmFIlfK18M'; // Web API key

async function seed() {
  console.log('🔥 Conectado a Firestore:', projectId);
  ...
  for (let i = 0; i < questions.length; i++) {
    const q = questions[i];
    const docId = q.id;
    const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/questions/${docId}?key=${apiKey}`;
    ...
    const body = {
      fields: {
        ...
        questionText: { stringValue: q.questionText },
        ...
        extraData: q.extraData ? { mapValue: { fields: serializeExtraData(q.extraData) } } : { nullValue: null },
      }
    };
    ...
    try {
      ...
      if (res.ok) {
        console.log(`  ✅ [${i + 1}/${questions.length}] ${q.id}: ${q.questionText.substring(0, 50)}...`);
      } else {
        const err = await res.text();
        console.error(`  ❌ [${i + 1}] ${q.id}: ${res.status} - ${err}`);
      }
    } catch (e) {
      console.error(`  ❌ [${i + 1}] ${q.id}: ${e.message}`);
    }
  }
...
function playerQuestions() {
  return [
    { id: 'p_001', type: 'player', difficulty: 'easy', questionText: '¿Qué jugador ha ganado más Balones de Oro?', correctAnswer: 'Lionel Messi', options: ['Lionel Messi', 'Cristiano Ronaldo', 'Michel Platini', 'Johan Cruyff'], imageUrl: null, extraData: { country: 'AR' } },
    { id: 'p_002', type: 'player', difficulty: 'easy', questionText: '¿En qué club jugó Messi la mayor parte de su carrera?', correctAnswer: 'FC Barcelona', options: ['FC Barcelona', 'Real Madrid', 'PSG', 'Inter Miami'], imageUrl: null, extraData: null },
    { id: 'p_003', type: 'player', difficulty: 'medium', questionText: '¿Qué jugador marcó el "Gol del Siglo" en México 86?', correctAnswer: 'Diego Maradona', options: ['Diego Maradona', 'Pelé', 'Zinedine Zidane', 'Ronaldo Nazário'], imageUrl: null, extraData: null },
    ...
    { id: 'p_005', type: 'player', difficulty: 'hard', questionText: '¿Qué jugador ganó el Balón de Oro en 1995?', correctAnswer: 'George Weah', options: ['George Weah', 'Roberto Baggio', 'Hristo Stoichkov', 'Jari Litmanen'], imageUrl: null, extraData: null },
    ...
    { id: 'p_011', type: 'player', difficulty: 'easy', questionText: '¿Qué jugador ganó 8 Balones de Oro?', correctAnswer: 'Lionel Messi', options: ['Lionel Messi', 'Cristiano Ronaldo', 'Pelé', 'Maradona'], imageUrl: null, extraData: null },
    { id: 'p_012', type: 'player', difficulty: 'medium', questionText: '¿Qué jugador ha jugado en más Mundiales?', correctAnswer: 'Antonio Carbajal', options: ['Antonio Carbajal', 'Pelé', 'Lothar Matthäus', 'Cristiano Ronaldo'], imageUrl: null, extraData: null },
...
function teamQuestions() {
  return [
    ...
    { id: 't_012', type: 'team', difficulty: 'medium', questionText: '¿Qué equipo español ha ganado más Ligas?', correctAnswer: 'Real Madrid', options: ['Real Madrid', 'Barcelona', 'Atlético de Madrid', 'Athletic Club'], imageUrl: null, extraData: null },
    { id: 't_013', type: 'team', difficulty: 'hard', questionText: '¿Qué equipo sudamericano ha ganado más Copas Libertadores?', correctAnswer: 'Independiente', options: ['Independiente', 'Boca Juniors', 'Peñarol', 'Santos'], imageUrl: null, extraData: null },
    { id: 't_014', type: 'team', difficulty: 'easy', questionText: '¿En qué país juega el Bayern Munich?', correctAnswer: 'Alemania', options: ['Alemania', 'Austria', 'Suiza', 'Países Bajos'], imageUrl: null, extraData: null },
...
function competitionQuestions() {
  return [
    ...
    { id: 'c_008', type: 'competition', difficulty: 'hard', questionText: '¿En qué año se jugó el primer Mundial de Fútbol?', correctAnswer: '1930', options: ['1930', '1928', '1934', '1926'], imageUrl: null, extraData: null },
...
function historyQuestions() {
  return [
    ...
    { id: 'h_007', type: 'history', difficulty: 'hard', questionText: '¿Quién ganó el primer Balón de Oro en 1956?', correctAnswer: 'Stanley Matthews', options: ['Stanley Matthews', 'Alfredo Di Stéfano', 'Ferenc Puskás', 'Raymond Kopa'], imageUrl: null, extraData: null },
...
function rulesQuestions() {
  return [
    ...
    { id: 'r_005', type: 'rules', difficulty: 'hard', questionText: '¿Cuántas tarjetas rojas significan la expulsión directa?', correctAnswer: '1', options: ['1', '2', '3', 'Depende de la falta'], imageUrl: null, extraData: null },
    ...
    { id: 'r_007', type: 'rules', difficulty: 'medium', questionText: '¿Cuántos cambios se permiten actualmente en partidos oficiales?', correctAnswer: '5', options: ['5', '3', '4', '6'], imageUrl: null, extraData: null },
    { id: 'r_008', type: 'rules', difficulty: 'hard', questionText: '¿Qué dimensiones tiene un campo de fútbol reglamentario?', correctAnswer: '105m x 68m', options: ['105m x 68m', '100m x 50m', '110m x 70m', '100m x 64m'], imageUrl: null, extraData: null },
...
function stadiumQuestions() {
  return [
    ...
    { id: 's_002', type: 'stadium', difficulty: 'easy', questionText: '¿En qué estadio jugaba el Barcelona?', correctAnswer: 'Camp Nou', options: ['Camp Nou', 'Santiago Bernabéu', 'Olímpico de Montjuïc', 'Wanda Metropolitano'], imageUrl: null, extraData: null },
    ...
    { id: 's_005', type: 'stadium', difficulty: 'hard', questionText: '¿Qué estadio es conocido como "La Bombonera"?', correctAnswer: 'Alberto J. Armando', options: ['Alberto J. Armando', 'Monumental', 'Centenario', 'Azteca'], imageUrl: null, extraData: null },
    ...
    { id: 's_007', type: 'stadium', difficulty: 'hard', questionText: '¿Cuál es el estadio más grande del mundo?', correctAnswer: 'Estadio Rungrado', options: ['Estadio Rungrado', 'Camp Nou', 'Maracaná', 'Wembley'], imageUrl: null, extraData: null },
    { id: 's_008', type: 'stadium', difficulty: 'medium', questionText: '¿En qué estadio juega el Liverpool?', correctAnswer: 'Anfield', options: ['Anfield', 'Old Trafford', 'Goodison Park', 'Stamford Bridge'], imageUrl: null, extraData: null },
...
function statisticQuestions() {
  return [
    ...
    { id: 'st_010', type: 'statistic', difficulty: 'hard', questionText: '¿Cuál es el récord de goles en una temporada de Premier League?', correctAnswer: '36 (Haaland, 2022-23)', options: ['36 (Haaland, 2022-23)', '34 (Cole, 1993-94)', '32 (Shearer, 1995-96)', '31 (Salah, 2017-18)'], imageUrl: null, extraData: null },
  ];
}
...
function transferQuestions() {
  return [
    ...
    { id: 'tr_002', type: 'transfer', difficulty: 'medium', questionText: '¿A qué club fue Mbappé gratis en 2024?', correctAnswer: 'Real Madrid', options: ['Real Madrid', 'Liverpool', 'Manchester United', 'Bayern Munich'], imageUrl: null, extraData: null },
    ...
    { id: 'tr_004', type: 'transfer', difficulty: 'medium', questionText: '¿Qué jugador fue del Barcelona al Real Madrid en 2000 por 60M€?', correctAnswer: 'Luis Figo', options: ['Luis Figo', 'Ronaldo Nazário', 'Michael Owen', 'David Beckham'], imageUrl: null, extraData: null },
    { id: 'tr_005', type: 'transfer', difficulty: 'hard', questionText: '¿Cuál fue el primer fichaje de más de 100M€?', correctAnswer: 'Gareth Bale (2013)', options: ['Gareth Bale (2013)', 'Cristiano Ronaldo (2018)', 'Neymar (2017)', 'Paul Pogba (2016)'], imageUrl: null, extraData: null },
    { id: 'tr_006', type: 'transfer', difficulty: 'easy', questionText: '¿A qué club fue Messi en 2021 tras el Barcelona?', correctAnswer: 'Paris Saint-Germain', options: ['Paris Saint-Germain', 'Inter Miami', 'Manchester City', 'Juventus'], imageUrl: null, extraData: null },
    ...
    { id: 'tr_009', type: 'transfer', difficulty: 'medium', questionText: '¿A qué fue Mbappé del Monaco al PSG inicialmente?', correctAnswer: 'Cedido con opción de compra', options: ['Cedido con opción de compra', 'Traspaso directo', 'Libre', 'Intercambio'], imageUrl: null, extraData: null },
